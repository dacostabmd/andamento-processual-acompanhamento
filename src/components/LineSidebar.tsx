...
/**
 * Sidebar de linhas com efeito de proximidade do cursor (estilo "magnetic
 * list"): item mais perto do ponteiro ganha cor de destaque, leve deslocamento
 * horizontal e escala do traço lateral. Aceita `activeIndex` controlado (além
 * de `defaultActive`) para sincronizar com scroll da página.
 */
const LineSidebar = ({
  items = DEFAULT_ITEMS,
  accentColor = '#A855F7',
  ...
  smoothing = 100,
  defaultActive = null,
  activeIndex: activeIndexControlado,
  onItemClick,
  className = '',
}: LineSidebarProps) => {
  ...
  const [activeIndexInterno, setActiveIndexInterno] = useState<number | null>(defaultActive)

  const activeIndex = activeIndexControlado ?? activeIndexInterno
